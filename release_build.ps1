...
try {
    ...
    $versionTitle = "TheraTrak Pro v$($version.major).$($version.minor).$($version.patch) Build $($version.build)"
...
"@
    ...
    if (-not $SkipRelease) {
        ...
        if (-not $DryRun) {
            Write-Host "Release published: https://github.com/Irish-Coder69/TheraTrak-Pro/releases/tag/$versionTag" -ForegroundColor Green
        }
    }
